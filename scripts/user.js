...
function fetchUsers (year) {
  ...
  const data = require(`../data/search/${year}.json`)

  // Use a midpoint if there's too many user's to fetch at a time
  // const midpoint = Math.floor(raw.length / 2)
  // const data = raw.splice(midpoint)

  // Get the user's login id
  const login = data.map(u => u.login)

  const fetchAll = Promise.resolve(login).map(fetchUser, { concurrency: 20 })
